...
- Last worked on: Angular 20 → 22 merged ([#482](https://github.com/jwilleke/yourphr/issues/482) / [#701](https://github.com/jwilleke/yourphr/pull/701), `161fd9c5`) — 701/701 tests, build, Storybook and CI all green on `main`.
- Branch / state: `main`, clean, nothing unpushed, no stashes.
- Running / in-flight: none. No background agents, dev servers or in-progress CI. No open PRs.
...
- Next steps:
  - File the Karma → Vitest migration issue — Angular 22 offers it as an official migration and it was deliberately skipped in [#701](https://github.com/jwilleke/yourphr/pull/701). Pairs with [#699](https://github.com/jwilleke/yourphr/issues/699) and [#702](https://github.com/jwilleke/yourphr/issues/702) as frontend-wide sweeps.
...
- Blockers / significant notes:
  - [#482](https://github.com/jwilleke/yourphr/issues/482) is `in-review` awaiting your close, as are ~10 others accumulated since the bulk close.
  - Worth a human glance: `ngx-highlightjs` jumped seven majors, so `language` is now required on all 17 `[highlight]` usages and the binary viewer moved to `[highlightAuto]`. Tests pass, but it is a rendering path nobody has looked at.
  - Code-scanning alert 68 (`js/clear-text-logging` in `scripts/lib/scrub.ts`) is still open and is yours to dismiss — it flags the scrubber itself.
<!-- RESUME:END -->
