...
- Last worked on: [#753](https://github.com/jwilleke/yourphr/issues/753) — the Epic sandbox connected but imported nothing (reported in [#749](https://github.com/jwilleke/yourphr/issues/749)). Fixed in `b174bfc93`: Patient read by id, a refused type skipped and named instead of aborting the sync, a `sync:` log line per sync. Released with [#739](https://github.com/jwilleke/yourphr/issues/739) and [#751](https://github.com/jwilleke/yourphr/issues/751) as [v3.5.2](https://github.com/jwilleke/yourphr/releases/tag/v3.5.2).
- Branch / state: `main`, tag `v3.5.2`; this pointer is the only change after it.
- Running / in-flight: the v3.5.2 image builds (`release-image.yaml`, `docker-relay-release.yaml`) were still running at hand-off. Check with `gh run list --workflow=release-image.yaml`, then confirm Flux rolled the pod to 3.5.2 (ssh deby, `sudo kubectl -n yourphr get deploy yourphr-ts -o jsonpath='{..image}'`).
- Parked / half-done: none. Restarting to get Claude in Chrome tools; they did not attach to the old session.
- Next steps:
  - __Verify #753 against the real Epic sandbox__ once 3.5.2 is live: in Chrome (select "jminm4"), sign in to the instance with the dedicated test login (not the real account), connect the Epic sandbox entry, log into MyChart as `fhircamila` (credentials in `private/secrets.md`). The Epic app accepts only the relay callback — a loopback redirect returns `error=4`, so `npm run live:smart` cannot be used. Then read the job on `/background-jobs` and the `sync: source N (…)` line in the pod log: which types imported, which Epic skipped (expect Observation/Condition 400 "requires a category").
  - Report the result on [#753](https://github.com/jwilleke/yourphr/issues/753); only then decide whether to reply to the reporter on #749 (Jim said no reply yet — #749 is closed without comment).
  - Confirm on demo.yourphr.org that the tour now gets the "needs full administrator access" message on Configuration ([#751](https://github.com/jwilleke/yourphr/issues/751)).
  - Close the in-review items when satisfied: [#739](https://github.com/jwilleke/yourphr/issues/739), [#740](https://github.com/jwilleke/yourphr/issues/740), [#741](https://github.com/jwilleke/yourphr/issues/741), [#742](https://github.com/jwilleke/yourphr/issues/742), [#743](https://github.com/jwilleke/yourphr/issues/743), [#751](https://github.com/jwilleke/yourphr/issues/751), [#753](https://github.com/jwilleke/yourphr/issues/753).
  - Optional follow-ups listed out of scope on #753, not yet filed: types from `/metadata`, `$everything`, Epic `category` params, per-type page budget, Epic scope prefill.
- Blockers / significant notes: Epic ignores requested scopes (the authorize redirect dropped `patient/*.read`) and grants per app registration, so 403s on ungranted types are expected and now skipped. Security alerts: Dependabot 0, code scanning 0. [#752](https://github.com/jwilleke/yourphr/issues/752) closed won't-fix (demo holds only admin/demo/demoadmin).
<!-- RESUME:END -->
...
- [#753](https://github.com/jwilleke/yourphr/issues/753) — [BUG] Epic sandbox connects but imports no records — v3 sync searches Patient?patient= and aborts on the first failing type (v2 parity)
