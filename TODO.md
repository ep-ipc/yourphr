...
- Last worked on: external report [#735](https://github.com/jwilleke/yourphr/issues/735) → file upload ([#736](https://github.com/jwilleke/yourphr/issues/736)) and C-CDA import restored in v3, released as [v3.5.0](https://github.com/jwilleke/yourphr/releases/tag/v3.5.0) (`ghcr.io/jwilleke/yourphr:3.5.0`); then every open PR merged, including a type fix on main so [#726](https://github.com/jwilleke/yourphr/pull/726) could land.
- Branch / state: `main`, clean, in sync with origin at `096c57e2f`; no stashes.
- Running / in-flight: none. CI green on `096c57e2f`; no open PRs; all monitors and agents finished.
- Parked / half-done: none. Local log `private/project_log.md` has entries `2026-09-14-01` and `-02` (gitignored).
- Next steps:
  - Fix [#739](https://github.com/jwilleke/yourphr/issues/739) — CodeQL high alert on `src/upload/index.ts:84` from today's release. False positive in context, but fix by tokenizing past comments/CDATA/PIs instead of `replace()`; the Go-parity fixtures in `src/upload/__tests__/upload.test.ts` prove behaviour is unchanged.
  - Manual pins, since Dependabot reports `security_update_not_possible`: frontend `js-yaml` ≥ 4.3.2 ([#740](https://github.com/jwilleke/yourphr/issues/740)), `csv-parse` 7.x under `@lhncbc/ucum-lhc` ([#743](https://github.com/jwilleke/yourphr/issues/743) — check ucum-lhc still loads its unit tables), `smol-toml` once patched ([#741](https://github.com/jwilleke/yourphr/issues/741)).
  - Close [#742](https://github.com/jwilleke/yourphr/issues/742) (hono) — in review, `yarn audit` clean.
  - On your own instance, after it runs 3.5.0: set `yourphr.cda-converter.url` on Admin → Configuration (probably `http://yourphr-cda-converter:8080`).
  - Triage [#737](https://github.com/jwilleke/yourphr/issues/737) (PDF/DICOM/image upload gone in v3).
- Blockers / significant notes: the reporter's leaked relay secret was checked on deby and is NOT yours; the two #735 revisions with their secrets are deleted from edit history. `CHANGELOG.md` stops at 3.3.0 — neither 3.4.0 nor 3.5.0 added an entry; the release notes carry the detail. `docker/setup-qemu-action` v4 ([#730](https://github.com/jwilleke/yourphr/pull/730)) and `metadata-action` v6 only run on a release tag — v3.5.0's image build used metadata-action v6 successfully; qemu v4 is untested until the next tag.
<!-- RESUME:END -->
