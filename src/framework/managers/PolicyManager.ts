...
import { isConfigObject, type ConfigObject } from '../../config/index.js';
...
/** A role: a flat, additive permission list, and whether this build defines it. */
export interface RoleDefinition {
  role: string;
  displayname: string;
  description: string;
  /** ngdpbase's `issystem` — a role the build defines, not the operator's to redefine away. */
  system: boolean;
...
export class PolicyManager extends BaseManager {
  ...
  roleDefinitions(): RoleDefinition[] { return [...this.roles.values()]; }
  ...
  async restore(): Promise<void> { /* restored with the configuration */ }
}

function readPermissions(raw: ConfigObject): Map<string, PermissionDefinition> {
...
function readRoles(raw: ConfigObject, permissions: Map<string, PermissionDefinition>): Map<string, RoleDefinition> {
  const out = new Map<string, RoleDefinition>();
  for (const [role, value] of Object.entries(raw)) {
    if (!isConfigObject(value)) throw new Error(`policy: ${ROLES_KEY} entry '${role}' is not an object`);
    ...
    if (unknown.length > 0) {
      ...
      throw new Error(
        `policy: role '${role}' in ${ROLES_KEY} grants ${unknown.length === 1 ? 'a permission' : 'permissions'} this build does not enforce — ${unknown.join(', ')}. ` +
