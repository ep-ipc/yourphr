 ...
 */
function resolveFastStorage(): string {
  if (process.env['YOURPHR_FAST_STORAGE']) return process.env['YOURPHR_FAST_STORAGE'];
  // Pre-yourphr#630 name, still honoured so an existing deployment keeps booting.
  if (process.env['YOURPHR_STORAGE_DATA_DIR']) return process.env['YOURPHR_STORAGE_DATA_DIR'];
  try {
