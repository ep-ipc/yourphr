...
import { BaseGlossaryProvider, type GlossaryEntry } from './BaseGlossaryProvider.js';

const ENDPOINT = 'https://connect.medlineplus.gov/service';

/** The subset of MedlinePlus's Atom-shaped JSON this needs. Everything else is ignored. */
interface MedlinePlusResponse {
  feed?: {
    author?: { name?: { _value?: string } };
    entry?: {
      title?: { _value?: string };
...
export class MedlinePlusGlossaryProvider extends BaseGlossaryProvider {
  readonly name = 'medlineplus';
  readonly available = true;
  readonly unavailableReason = '';

  constructor(private readonly options: { allowInternal?: boolean; timeoutMs?: number } = {}) {
    super();
  }

  async explain(code: string, codeSystemOid: string): Promise<GlossaryEntry | undefined> {
    ...
    let parsed: MedlinePlusResponse;
    try {
      parsed = JSON.parse(response.body.toString('utf8')) as MedlinePlusResponse;
    } catch (err) {
      throw new Error(`glossary: MedlinePlus returned something that is not JSON (${(err as Error).message})`);
    }
