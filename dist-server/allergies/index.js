 ...
 */
import { conceptCode, conceptText, noteText, refIsType, standardCodingsBy } from '../conditions/index.js';
...
function noKnown(raw) {
    if (!raw.code)
        return false;
    for (const c of raw.code.coding ?? []) {
        if (NO_KNOWN_ALLERGY_CODES.has(String(c?.code ?? '').trim()))
            return true;
    }
    return conceptText(raw.code).toLowerCase().includes('no known');
}
const onsetOf = (raw) => raw.onsetDateTime || raw.onsetPeriod?.start || raw.onsetString || '';
function reactionsOf(raw) {
    const out = [];
    for (const rx of raw.reaction ?? []) {
        const manifestations = (rx?.manifestation ?? []).map((m) => conceptText(m)).filter((s) => s !== '');
        const description = String(rx?.description ?? '');
        const severity = String(rx?.severity ?? '').toLowerCase();
        if (manifestations.length === 0 && description === '' && severity === '')
            continue;
        out.push({
...
function resolveState(clinical, verif) {
    if (verif === 'refuted')
        return 'RuledOut';
    switch (clinical) {
...
function verificationLabel(verif) {
    switch (verif) {
...
const recency = (c) => firstNonEmpty(c.recorded ?? '', c.end ?? '', c.start ?? '');
function mergeGroup(g) {
    let rep = g[0];
    for (const c of g.slice(1))
        if (recency(c) > recency(rep))
            rep = c;
    const merged = { ...rep, occurrences: g.length };
    ...
    if (end) {
        merged.end = end;
        merged.lastActivity = end;
    }
    else {
        delete merged.end;
        delete merged.lastActivity;
    }
    if (known)
        merged.noKnown = true;
    else
        delete merged.noKnown;
...
export function classifyAllergies(resources) {
    const order = [];
