 ...
 */
export function sourceShape(source, latestJob) {
    const redact = (secret) => (secret === '' ? '' : '[REDACTED]');
    return {
        id: `source-${source.id}`,
...
export function isDisconnected(source) {
...
export class SourcesManager extends BaseManager {
    ...
    async add(ctx, source) {
        ...
        if (source.userId !== ctx.username)
            throw new ApiError(403, 'a source can only be connected for the signed-in account');
        return this.provider.add(source);
    }
    /** Disconnect (Go's #437): the tokens go, the records stay; the worker skips it. */
    async disconnect(ctx, publicId) {
        const s = await this.get(ctx, publicId);
        if (!s)
            return false;
     ...
     */
    async disconnectWhere(ctx, predicate) {
        let disconnected = 0;
        for (const s of await this.list(ctx)) {
            if (!predicate(s) || isDisconnected(s))
                continue;
            await this.provider.clearTokens(s.id);
            disconnected++;
        }
    ...
    async exportBundle(ctx, publicId) {
        const s = await this.get(ctx, publicId);
        if (!s)
            return undefined;
    ...
    async saveDynamicClient(ctx, sourceId, client) {
        if (!(await this.owned(ctx, sourceId)))
            throw new ApiError(404, 'source not found');
        await this.provider.saveDynamicClient(sourceId, client);
    }
    async dynamicClientFor(ctx, sourceId) {
        if (!(await this.owned(ctx, sourceId)))
            return undefined;
     ...
     */
    async syncNow(ctx, publicId, now = Math.floor(Date.now() / 1000)) {
        const s = await this.get(ctx, publicId);
        if (!s)
            return undefined;
        const job = await this.syncOne(ctx, s, now);
        if (job.outcome !== 'success')
            throw new ApiError(502, job.error || 'sync failed');
    ...
    syncInBackground(ctx, source) {
        void this.syncOne(ctx, source).catch((err) => this.log(`initial sync failed for source ${source.id}: ${err.message}`));
    }
    /**
     ...
     */
    async pass(now = Math.floor(Date.now() / 1000)) {
        const report = { refreshAttempted: 0, refreshed: 0, synced: 0, failed: 0 };
    ...
    async syncOne(ctx, source, now = Math.floor(Date.now() / 1000), report) {
        ...
        try {
            ...
            if (source.expiresAt < now + REFRESH_MARGIN_SECONDS) {
                ...
                if (source.refreshToken === '') {
                    ...
                    try {
                        ...
                        if (fresh.tokenUrl !== source.tokenUrl)
                            await this.provider.updateTokenUrl(source.id, fresh.tokenUrl); // discovered once, persisted (yourphr#584)
                        ...
                        this.log(`token-refresh: source ${source.id} (${source.display}): ${err.message}`);
                    }
                }
            }
            ...
            try {
                ...
                job = { sourceId: source.id, outcome: 'failure', received, created, updated, error: err.message.slice(0, 512), startedAt: now, finishedAt: now };
            }
     ...
     */
    async importLegacy(ctx, sources) {
        if (ctx.system === '')
            throw new ApiError(403, 'legacy import is the migration tool\'s alone');
        const report = { imported: [], skippedExisting: [], needsReconnect: [], idMap: {} };
