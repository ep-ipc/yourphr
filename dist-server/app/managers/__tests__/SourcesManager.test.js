...
import { SourcesManager, isDisconnected, sourceShape } from '../SourcesManager.js';
...
import { BaseSourceClientProvider, NullSourceClientProvider } from '../../providers/BaseSourceClientProvider.js';
...
class ScriptedClient extends BaseSourceClientProvider {
    ...
    failFetch = false;
    perType = 2;
    async beginAuthorization() { throw new Error('not in this spec'); }
    async completeAuthorization() { throw new Error('not in this spec'); }
    ...
    async fetchPages(source, resourceType, accessToken, writer) {
        this.fetches.push(`${source.id}:${resourceType}:${accessToken}`);
        if (this.failFetch)
            throw new Error('FHIR HTTP 500');
        let created = 0;
        let updated = 0;
        for (let i = 1; i <= this.perType; i++) {
            const r = await writer.upsert({ resourceType, id: `${resourceType.toLowerCase()}-${source.patient}-${i}`, code: { text: `synthetic ${resourceType}` } });
            if (r === 'created')
                created++;
            else
                updated++;
...
describe('SourcesManager — the operator and the migration tool', () => {
    it('admin metrics are for admins, keyed by outcome|platform|environment, newest jobs first', async () => {
        ...
        const recent = metrics['recent_jobs'];
...
//# sourceMappingURL=SourcesManager.test.js.map