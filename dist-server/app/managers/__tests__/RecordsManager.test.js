...
const obs = (id, code, display, date, system = LOINC) => ({ resourceType: 'Observation', id, status: 'final', code: { coding: [{ system, code, display }] }, effectiveDateTime: date });
let provider;
let favorites;
let engine;
let records;
...
beforeEach(async () => {
    ...
    provider.seed('alice', 'source-1', { resourceType: 'Condition', id: 'c1', code: { text: 'Hypertension', coding: [{ system: SNOMED, code: '38341003' }] }, clinicalStatus: { coding: [{ code: 'active' }] }, recordedDate: '2024-07-01' });
    provider.seed('alice', '', { resourceType: 'Condition', id: 'c2', code: { text: 'Typed in' } });
...
describe('RecordsManager — the one door, scoped to whoever is asking', () => {
    ...
    it('the typed query: grouped by code with max(sort_date), system-only tokens, count_by, and refusals', async () => {
        const grouped = (await records.query(alice, { from: 'Observation', where: { code: `${LOINC}|` }, aggregations: { order_by: { field: 'sort_date', fn: 'max' }, group_by: { field: 'code' } } }));
        expect(grouped).toEqual([{ label: `${LOINC}|718-7`, value: '2024-05-10' }, { label: `${LOINC}|2345-7`, value: '2024-03-10' }]);
        const rows = (await records.query(alice, { from: 'Observation', where: { code: '718-7,2345-7' } }));
        expect(rows.map((r) => r['source_resource_id'])).toEqual(['o2', 'o3', 'o1']);
        const counted = (await records.query(alice, { from: 'Observation', aggregations: { count_by: { field: 'code' } } }));
    ...
    it('the MedicalHistory graph: each requested encounter with everything reachable through references, both ways, Binary excluded, newest first', async () => {
        ...
        provider.seed('alice', 'source-1', { resourceType: 'Encounter', id: 'e1', period: { start: '2024-03-01' }, participant: [{ individual: ref('Practitioner/dr-1') }], serviceProvider: ref('Organization/org-1'), diagnosis: [{ condition: ref('Condition/c1') }] });
        provider.seed('alice', 'source-1', { resourceType: 'Practitioner', id: 'dr-1', name: [{ text: 'Dr One' }] });
        provider.seed('alice', 'source-1', { resourceType: 'Organization', id: 'org-1', name: 'Clinic' });
        provider.seed('alice', 'source-1', { resourceType: 'Observation', id: 'ob-e1', status: 'final', code: { text: 'BP' }, effectiveDateTime: '2024-03-02', encounter: ref('Encounter/e1') });
        provider.seed('alice', 'source-1', { resourceType: 'DocumentReference', id: 'doc-1', status: 'current', date: '2024-03-01', context: { encounter: [ref('Encounter/e1')] }, content: [{ attachment: { url: 'Binary/bin-1' } }] });
        provider.seed('alice', 'source-1', { resourceType: 'Binary', id: 'bin-1', contentType: 'text/plain' });
        provider.seed('alice', 'source-1', { resourceType: 'Encounter', id: 'e2', period: { start: '2024-05-01' }, reasonReference: [ref('Condition/missing')] });
        provider.seed('bob', 'source-9', { resourceType: 'Encounter', id: 'e-bob', period: { start: '2024-01-01' } });
        const ids = [
        ...
        const relatedIds = e1['related_resources'].map((r) => `${r['source_resource_type']}/${r['source_resource_id']}`);
        ...
        expect(e1['related_resources'][0]).toMatchObject({ source_id: 'source-1', resource_raw: expect.objectContaining({ resourceType: 'Condition', id: 'c1' }) });
        expect(encounters[0]['related_resources']).toEqual([]); // a dangling reference is not invented
    ...
    it('find anything by words: every word must match the record\'s own text, best first with a snippet; user A never sees user B', async () => {
        provider.seed('alice', 'source-1', { resourceType: 'MedicationStatement', id: 'm1', status: 'active', medicationCodeableConcept: { text: 'Metformin 500 MG oral tablet', coding: [{ system: 'http://www.nlm.nih.gov/research/umls/rxnorm', code: '860975' }] }, effectiveDateTime: '2023-06-01', note: [{ text: 'take with the evening meal' }] });
        provider.seed('alice', 'source-1', { resourceType: 'DocumentReference', id: 'doc-c', status: 'current', date: '2023-02-10', type: { text: 'Cardiology consult note' }, description: 'Follow-up after the stress test' });
        provider.seed('bob', 'source-9', { resourceType: 'MedicationStatement', id: 'm-bob', status: 'active', medicationCodeableConcept: { text: 'Metformin 1000 MG' } });
...
//# sourceMappingURL=RecordsManager.test.js.map