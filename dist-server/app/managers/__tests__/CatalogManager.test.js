...
import { BaseSourceClientProvider, NullSourceClientProvider, SourceClientError } from '../../providers/BaseSourceClientProvider.js';
...
class ScriptedClient extends BaseSourceClientProvider {
    ...
    async beginAuthorization(app, redirectUri) {
        ...
        if (this.failDiscovery)
            throw new SourceClientError('discovery', 'SMART discovery failed: no .well-known');
    ...
    async completeAuthorization(app, redirectUri, code, codeVerifier) {
        ...
        if (this.failExchange)
            throw new SourceClientError('exchange', 'token exchange failed: HTTP 400');
        return { tokenUrl: 'https://idp.example.org/token', accessToken: 'at', refreshToken: 'rt', expiresAt: 2_000, patient: this.patient };
    }
    async refresh() { throw new Error('not in this spec'); }
    async fetchPages() { return { received: 0, created: 0, updated: 0 }; }
}
const PROD = { display: 'Big Hospital', environment: 'production', fhirBaseUrl: 'https://fhir.example.org/r4', scopes: 'patient/Condition.read patient/Observation.read', clientId: 'cid', clientSecret: 'the-secret', enabled: true };
const SANDBOX = { display: 'Sandbox', environment: 'sandbox', fhirBaseUrl: 'https://sandbox.example.org/r4', scopes: 'patient/*.read', clientId: 'sb', enabled: true };
let dir;
let engine;
let provider;
let client;
...
let alice;
let seed;
async function boot(sourceClient) {
    engine = new Engine();
...
describe('CatalogManager — the admin curates', () => {
    ...
    it('seeds provision-then-preserve: creates missing, fills empty credentials, never clobbers an edit, skips a bad seed with a log line', async () => {
        ...
        const idOf = async (display) => (await catalog.entries(admin)).find((e) => e.display === display).id;
    ...
    it('legacy import is the migration principal\'s alone, one-way, reporting rejections by reason', async () => {
        await catalog.createEntry(admin, PROD);
        const legacy = [PROD, SANDBOX, { ...SANDBOX, display: 'Weird', environment: 'staging' }, { ...SANDBOX, display: 'Loopback', fhirBaseUrl: 'http://127.0.0.1:9999/fhir' }];
...
//# sourceMappingURL=CatalogManager.test.js.map