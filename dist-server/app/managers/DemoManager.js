...
export class DemoManager extends BaseManager {
    log;
    name = 'demo';
    dependsOn = ['configuration', 'users', 'sessions'];
    ...
    refuseConnect(ctx) {
        if (!this.isDemoSession(ctx))
            return;
        throw new ApiError(403, 'connecting a provider is disabled in the public demo — the demo account is shared, so records imported here would be visible to everyone', { code: DEMO_ERROR_CODE });
    }
    /**
     ...
     */
    async signIn() {
        const refused = { ok: false, error: 'demo mode is not configured on this instance' };
        if (!this.enabled)
            return { ok: false, error: 'demo mode is not enabled on this instance' };
    ...
    async provision() {
        ...
        if (existing !== '' && (await this.engine.managers.users.passwordMatches(username, existing)))
            return;
        ...
        try {
            ...
            this.log(`demo credential: set a new password on ${JSON.stringify(username)} but could not record it in ${PASSWORD_KEY} (${err.message})`);
            return;
        }
