...
import { SourceClientError } from '../providers/BaseSourceClientProvider.js';
...
export class CatalogManager extends BaseManager {
    ...
    options;
    name = 'catalog';
    dependsOn = ['users', 'sources'];
    constructor(engine, provider, client, options = {}) {
        super(engine);
    ...
    checkUrls(fields, allowInternal = this.options.allowInternal ?? false) {
        for (const [field, value] of [['fhirBaseUrl', fields.fhirBaseUrl], ['authorizeUrlOverride', fields.authorizeUrlOverride]]) {
            if (value === '' && field === 'authorizeUrlOverride')
                continue;
            const checked = validateUrl(value, allowInternal);
            if (!checked.ok)
                throw new ApiError(400, `${field} rejected: ${checked.reason}`);
            if (fields.environment === 'production' && checked.url.protocol !== 'https:')
                throw new ApiError(400, `${field} must be https for a production provider`);
        }
    }
    /** A write over what is stored (or nothing): the write-only secret rule and Go's normalisations. */
    merge(write, existing, existingSecret = '') {
        return {
    ...
    manage(ctx) {
        if (ctx.system === '')
            ctx.require('admin-system', MANAGE);
    }
    async entryById(publicId) {
        return /^\d+$/.test(publicId) ? this.provider.byId(Number(publicId)) : undefined;
    }
    async enabledEntry(publicId) {
        const e = await this.entryById(publicId);
        if (!e || !e.enabled)
            throw new ApiError(404, 'no such enabled catalog entry');
        return e;
    }
    /** Go's providerCatalogRequest -> a write; an update keeps what the body does not say. */
    static writeFromBody(body, existing) {
        const str = (k, fallback = '') => (typeof body[k] === 'string' ? body[k].trim() : fallback);
        return {
    ...
    async create(ctx, body) {
        ...
        if (!write.display || !write.fhirBaseUrl || !write.clientId)
            throw new ApiError(400, 'display, api_endpoint_base_url, and client_id are required');
        return catalogEntryShape(await this.createEntry(ctx, write));
    }
    async updateEntry(ctx, id, write) {
        this.manage(ctx);
    ...
    async update(ctx, publicId, body) {
        this.manage(ctx);
        const e = await this.entryById(publicId);
        if (!e)
            return undefined;
    ...
    asApiError(err) {
        if (err instanceof ApiError)
            return err;
        if (err instanceof SourceClientError)
            return new ApiError(err.stage === 'unavailable' ? 501 : 502, err.message);
        return new ApiError(502, err.message);
    }
    /** Go's authorize answer: where to send the member, and what the callback must bring back. */
    async authorize(ctx, publicId, body) {
        ctx.requireAuthenticated();
        ...
        if (this.engine.has('demo'))
            this.engine.managers.demo.refuseConnect(ctx);
        ...
        if (redirectUri === '')
            throw new ApiError(501, 'no SMART relay in this stack — supply redirect_uri (this deployment\'s /sources/callback/<state> page)');
        try {
    ...
    async connect(ctx, publicId, body) {
        ctx.requireAuthenticated();
        const e = await this.enabledEntry(publicId);
        if (connectionPolicy(e).requiresUserConsent && (await this.engine.managers.users.consentAcceptedAt(ctx)) === '') {
            throw new ApiError(403, 'Accept the Privacy Policy and Terms of Service on Account Profile before connecting a medical source.', { error_code: 'legal_consent_required', privacy_policy_url: '/privacy', terms_of_service_url: '/terms' });
        }
        const str = (k) => (typeof body[k] === 'string' ? body[k].trim() : '');
        const verifier = str('code_verifier');
        const code = str('code');
        if (verifier === '')
            throw new ApiError(400, 'code_verifier is required');
        if (code === '' && str('state') === '')
            throw new ApiError(400, 'one of code or state is required');
        if (code === '')
            throw new ApiError(501, 'no SMART relay in this stack — the callback page must send the authorization code');
        const redirectUri = str('redirect_uri');
        if (redirectUri === '')
            throw new ApiError(400, 'redirect_uri is required (the one the authorization used)');
        let granted;
        try {
        ...
        if (granted.patient === '')
            throw new ApiError(502, 'token had no patient id — this stack does not yet resolve one from the FHIR API');
        const patientFacing = connectableShape(e)['display'];
        ...
        for (const s of seeds) {
            ...
            if (!existing) {
                ...
                try {
                    ...
                    this.options.log?.(`catalog seed "${s.display}" skipped: ${err.message}`);
                    continue;
                }
    ...
    async importLegacy(ctx, entries, options = {}) {
        if (ctx.system === '')
            throw new ApiError(403, 'legacy import is the migration tool\'s alone');
        const report = { imported: [], skippedExisting: [], rejected: [] };
        ...
        for (const e of entries) {
            ...
            try {
                ...
                report.rejected.push({ display: e.display, reason: err.message });
            }
        }
        return report;
    }
