...
export function textFor(resource) {
    const out = [];
    const walk = (node, key) => {
        if (out.join(' ').length > MAX_CHARS)
            return;
        ...
        if (node && typeof node === 'object') {
            for (const [k, v] of Object.entries(node)) {
                if (SKIP_KEYS.has(k))
                    continue;
                if (k === 'div' && typeof v === 'string') {
                    out.push(stripTags(v));
                    continue;
                }
                walk(v, k);
            }
        ...
        if (typeof node === 'string') {
            if (key === 'resourceType')
                out.push(node.replace(/([a-z])([A-Z])/g, '$1 $2'));
            else if (TEXT_KEYS.has(key))
                out.push(node);
            else if (DATE_KEYS.has(key))
                out.push(node.slice(0, 10));
        }
...
//# sourceMappingURL=record-text.js.map