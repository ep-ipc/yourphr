...
const obs = (id, code, date, system = LOINC) => ({ resourceType: 'Observation', id, status: 'final', code: { coding: [{ system, code, display: code }] }, effectiveDateTime: date });
let dir;
let provider;
beforeEach(async () => {
...
describe('SqliteRecordsProvider — PHI storage over SQLCipher, scoped per account', () => {
    ...
    it('indexed search: exact token, system|code, system-only prefix, OR within a parameter, AND across; grouped values', async () => {
        const ids = async (where) => (await provider.indexedSearch('alice', 'Observation', where)).map((r) => r.id).sort();
...
describe('SqliteRecordsProvider — find anything by words (yourphr#599)', () => {
    it('indexes the record\'s own text into FTS5 on the same write, searches per owner with a snippet, and forgets a deleted record', async () => {
        ...
        await alice.upsert({ resourceType: 'MedicationStatement', id: 'm1', status: 'active', medicationCodeableConcept: { text: 'Metformin 500 MG oral tablet' }, effectiveDateTime: '2023-06-01' });
        await alice.upsert({ resourceType: 'DocumentReference', id: 'd1', status: 'current', type: { text: 'Cardiology consult note' }, date: '2023-02-10' });
        await bob.upsert({ resourceType: 'MedicationStatement', id: 'm9', status: 'active', medicationCodeableConcept: { text: 'Metformin 1000 MG' } });
...
//# sourceMappingURL=SqliteRecordsProvider.test.js.map