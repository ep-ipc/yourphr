...
import { sseFrame } from './events/index.js';
 ...
 */
export function toResourceFhir(resource, sourceId) {
    return {
        source_id: sourceId,
...
export function titleFor(resource) {
...
export function dateFor(resource) {
 ...
 */
function serveStatic(webDir, pathname, res) {
...
export function createYourPhrServer(options) {
    ...
    const engineReady = options.engine
        ? Promise.resolve(options.engine)
        : (async () => {
            if (!options.repo)
                throw new Error('createYourPhrServer needs an engine or a repository');
            const e = new Engine();
    ...
    let authLimiter;
    let limiterSettings = '';
    const limiterFor = (engine) => {
        if (!engine.has('configuration'))
            return authLimiter;
    ...
    return createServer(async (req, res) => {
        try {
            ...
            if (auth && url.pathname.startsWith('/api/secure/')) {
                ...
                const session = token ? await engine.managers.sessions.verify(token) : { ok: false };
                if (!session.ok) {
            ...
            if (auth && engine.has('audit') && req.method === 'GET') {
                ...
                if (category)
                    await engine.managers.audit.record(ctx, category);
            }
            ...
            if (auth && url.pathname.startsWith('/api/secure/account/')) {
                ...
                if (url.pathname === '/api/secure/account/password' && req.method === 'POST') {
                    ...
                    if (engine.has('demo'))
                        engine.managers.demo.refuseWrite(ctx, 'changing the password');
                    const body = await readJsonBody(req);
                ...
                if (url.pathname === '/api/secure/account/sign-out-everywhere' && req.method === 'POST') {
                    ...
                    if (engine.has('demo'))
                        engine.managers.demo.refuseWrite(ctx, 'signing out everywhere');
                ...
                if (url.pathname === '/api/secure/account/me' && req.method === 'DELETE') {
                    ...
                    if (engine.has('demo'))
                        engine.managers.demo.refuseWrite(ctx, 'deleting the account');
                    ...
                    if (engine.has('sources'))
                        await engine.managers.sources.removeAll(ctx);
                    await engine.managers.records.removeAll(ctx);
                    if (engine.has('audit'))
                        await engine.managers.audit.removeForUser(ctx);
            ...
            if (engine.has('catalog') && url.pathname.startsWith('/api/secure/provider-catalog')) {
                ...
                if (url.pathname !== '/api/secure/provider-catalog/connectable') {
                    ...
                    if (!ctx.can('admin-system')) {
                        send(res, 403, { success: false, error: 'admin role required to manage the provider catalog' });
                        return;
                    }
                    ...
                    if (url.pathname === '/api/secure/provider-catalog' && req.method === 'POST') {
                        ...
                        try {
                            send(res, 200, { success: true, data: await cat.create(ctx, body) });
                        }
                        catch (err) {
                            fail(err);
                        }
            ...
            if (engine.has('sources')) {
                ...
                if (src.events && url.pathname === '/api/secure/events/stream' && req.method === 'GET') {
                    ...
                    const close = () => { clearInterval(keepAlive); unsubscribe(); res.end(); };
                    req.on('close', close);
                    res.on('error', close);
                    return;
                }
                ...
                if (sourceMatch) {
                    ...
                    const notFound = () => send(res, 404, { success: false, error: 'source not found' });
                    ...
                    if (action === 'sync' && req.method === 'POST') {
                        ...
                        try {
                            ...
                            send(res, 500, { success: false, error: `record sync failed: ${err.message}` });
                            return;
                        }
            ...
            // --- the dashboard and record pages (yourphr#595) ---
            {
                ...
                if (url.pathname === '/api/secure/query' && req.method === 'POST') {
                    ...
                    try {
                        ...
                        send(res, err instanceof ApiError ? err.status : 400, { success: false, error: err.message });
                    }
                    return;
                }
            }
            ...
            if (url.pathname === '/api/secure/user/favorites') {
                ...
                if (req.method === 'POST' || req.method === 'DELETE') {
                    const body = await readJsonBody(req);
                    const str = (k) => (typeof body?.[k] === 'string' ? body[k] : '');
                    const fav = { source_id: str('source_id'), resource_type: str('resource_type'), resource_id: str('resource_id') };
                    if (req.method === 'POST')
                        send(res, 200, { success: true, data: await records.addFavorite(ctx, fav) });
                    else
                        send(res, 200, { success: true, data: { removed: await records.removeFavorite(ctx, fav) } });
                    return;
                }
            }
            if (url.pathname.startsWith('/api/secure/admin/')) {
                ...
                const backupFailed = (err) => send(res, err.status ?? 400, { success: false, error: err.message });
                ...
                if (backups && url.pathname === '/api/secure/admin/database/schedule' && req.method === 'POST') {
                    ...
                    try {
                        send(res, 200, { success: true, data: backups.setSchedule(ctx, body) });
                    }
                    catch (err) {
                        backupFailed(err);
                    }
                ...
                if (backups && url.pathname === '/api/secure/admin/database/backup/test' && req.method === 'POST') {
                    const body = await readJsonBody(req);
                    send(res, 200, { success: true, data: await backups.testDestination(ctx, typeof body?.['destination'] === 'string' ? body['destination'] : '') });
                    return;
                }
                ...
                if (backups && url.pathname === '/api/secure/admin/database/restore' && req.method === 'POST') {
                    const body = await readJsonBody(req);
                    const name = typeof body?.['backup_name'] === 'string' ? body['backup_name'] : '';
                    if (!body || name === '') {
                        send(res, 400, { success: false, error: 'invalid request' });
                        return;
                    }
            ...
            const attributed = (row) => (row['source_id'] === '' ? { ...row, source_id: fallbackSource } : row);
            ...
            if (graphMatch && req.method === 'POST') {
                ...
                const ids = Array.isArray(body['resource_ids']) ? body['resource_ids'] : [];
                ...
                for (const list of Object.values(graph.results)) {
                    for (const row of list) {
                        ...
                        row['related_resources'] = row['related_resources'].map(attributed);
                    }
                }
