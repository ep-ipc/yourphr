...
import { addColumnWithDefault } from './framework/providers/sqlite-migrations.js';
...
import { CatalogManager } from './app/managers/CatalogManager.js';
...
import { NullSourceClientProvider } from './app/providers/BaseSourceClientProvider.js';
import { NullGlossaryProvider } from './app/providers/BaseGlossaryProvider.js';
...
import { NullBackupProvider } from './framework/providers/BaseBackupProvider.js';
...
const APP_MIGRATIONS = [
    ...
    },
    {
        ...
        created_at TEXT NOT NULL
            ...
            const columns = db.pragma('table_info(auth_users)').map((c) => c.name);
    ...
    },
    {
        ...
        last_sync_at INTEGER NOT NULL DEFAULT 0
            ...
            const columns = db.pragma('table_info(connected_sources)').map((c) => c.name);
            if (!columns.includes('platform_type'))
                addColumnWithDefault(db, 'connected_sources', 'platform_type', 'TEXT', '');
            if (!columns.includes('environment'))
                addColumnWithDefault(db, 'connected_sources', 'environment', 'TEXT', '');
        },
    },
    {
        id: '20260822150000',
        ...
        authorize_url_override TEXT NOT NULL DEFAULT ''
            ...
            const columns = db.pragma('table_info(provider_catalog)').map((c) => c.name);
            for (const column of ['platform_type', 'brand_logo_url', 'consent_policy', 'pre_connect_profile']) {
                if (!columns.includes(column))
                    addColumnWithDefault(db, 'provider_catalog', column, 'TEXT', '');
            }
        },
    },
];
/** The backup-storage provider configuration names: 'filesystem' or 'null' (inert, said so); anything else refuses to boot. */
function backupProviderFor(name) {
    if (name === 'filesystem')
        return new FilesystemBackupProvider(BACKUP_SUFFIX);
    if (name === 'null') {
        appLog.warn('backup.storage.provider = null: no backup storage — every backup action will refuse');
        return new NullBackupProvider();
    }
...
function auditProviderFor(name, db) {
    if (name === 'sqlite')
        return new SqliteAuditProvider(db);
 ...
 */
async function glossaryProviderFor(name, env) {
    if (name === 'null')
        return new NullGlossaryProvider();
    if (name === 'medlineplus') {
...
async function sourceClientFor(name, env) {
    if (name === 'null')
        return new NullSourceClientProvider();
    if (name === 'smart') {
...
export async function openStores(dataDir, env = process.env) {
...
export async function assembleApp(dataDir, options = {}) {
    ...
    const systemCtx = (name, username) => ApiContext.system(name, username, engine);
    ...
    const scheduleTimer = options.workerIntervalMs === undefined ? undefined : setInterval(() => {
        const now = new Date();
        if (!backups.due(now, lastScheduledMinute))
            return;
        lastScheduledMinute = now.toISOString().slice(0, 16);
        backupNow().then((r) => appLog.info(`scheduled backup written: ${r.file} (${r.sizeBytes} bytes)`), (err) => appLog.error(`scheduled backup failed: ${err.message}`));
    }, 60_000);
    scheduleTimer?.unref?.();
