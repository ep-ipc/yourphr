...
export function isBlockedIp(address) {
    ...
    if (v4) {
        const [a, b] = v4;
        if (a === 0)
            return true; // 0.0.0.0/8, includes the unspecified address
        if (a === 10)
            return true; // RFC1918
        if (a === 127)
        ...
        if (a === 169 && b === 254)
            return true; // link-local, includes 169.254.169.254 metadata
        if (a === 172 && b >= 16 && b <= 31)
            return true; // RFC1918
        if (a === 192 && b === 168)
            return true; // RFC1918
    ...
    if (/^f[cd]/.test(ip))
        return true; // fc00::/7 unique-local, includes fd00:ec2::254 metadata
    if (/^fe[89ab]/.test(ip))
 ...
 */
export function guardedLookup(allowInternal = false) {
    const guarded = (hostname, options, callback) => {
        ...
        dnsLookup(hostname, { ...opts, all: true }, (err, addresses) => {
            ...
            if (!allowInternal) {
                ...
                if (blocked) {
                    cb(Object.assign(new Error(`${REFUSAL}: ${hostname} resolved to ${blocked.address}`), {
                        code: 'ESSRFBLOCKED',
                    }));
                    return;
                }
...
export function guardedAgents(allowInternal = false) {
 ...
 */
export function validateUrl(raw, allowInternal = false) {
    let url;
    try {
        url = new URL(raw);
    }
