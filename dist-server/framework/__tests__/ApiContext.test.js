...
describe('ApiContext — who is asking, immutable, with guards that throw ApiError', () => {
    ...
    it('guards: anonymous is 401, a member is 403 on requireAdmin, an admin passes', () => {
        ...
        try {
            nobody.requireAuthenticated();
        }
        catch (e) {
            expect(e.status).toBe(401);
        }
        try {
            member.require('admin-read');
        }
        catch (e) {
            expect(e.status).toBe(403);
            expect(e.message).toBe('admin role required');
        }
...
//# sourceMappingURL=ApiContext.test.js.map