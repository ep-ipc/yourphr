...
import { envNameFor, PUBLIC_KEYS_KEY } from '../../config/index.js';
...
export class SettingsManager extends BaseManager {
     ...
     */
    legalDocument(_ctx, kind) {
        const parsed = parseLegalKind(kind);
        if (!parsed)
            return undefined;
        if (this.dataDir === undefined)
            throw new ApiError(500, 'legal documents are not configured on this instance');
    ...
    configReveal(ctx, key) {
        ctx.require('admin-system');
        if (!this.configuration.keys().includes(key))
            return undefined;
    ...
    configSet(ctx, key, value) {
        ...
        if (!config.keys().includes(key))
            throw new ApiError(400, `unknown configuration key ${JSON.stringify(key)} — only keys this build ships can be set`);
        ...
        try {
            ...
            throw new ApiError(400, `${key}: ${err.message}`);
        }
        try {
    ...
    configReset(ctx, key) {
        ctx.require('admin-system');
        if (!this.configuration.keys().includes(key))
            throw new ApiError(400, `unknown configuration key ${JSON.stringify(key)}`);
        let cleared;
        try {
        ...
        if (cleared)
            this.log(`${ctx.actor} reset configuration ${key} to its default`);
        return cleared;
    }
    instanceSettings(ctx) {
        ctx.require('admin-read');
        const config = this.configuration;
        return {
    ...
    setInstanceSettings(ctx, s) {
        ctx.require('admin-system');
        const settings = { name: s.name.trim(), contact_email: s.contact_email.trim(), contact_url: s.contact_url.trim() };
...
export function coerceToShippedType(value, shipped) {
