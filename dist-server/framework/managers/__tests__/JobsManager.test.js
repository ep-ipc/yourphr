...
let alice;
let admin;
const job = (sourceId, outcome, received = 3, error = '') => ({ sourceId, outcome, received, created: received, updated: 0, error, startedAt: 1_000, finishedAt: 1_002 });
beforeEach(async () => {
...
describe('JobsManager — the history of background runs', () => {
    ...
    it('shapes durations in ms and carries the error into the summary only when there is one', () => {
        ...
        expect(ok['data'].summary).not.toHaveProperty('error_message');
        expect(ok['data']).not.toHaveProperty('error_data');
    });
});
//# sourceMappingURL=JobsManager.test.js.map