...
export async function registerDynamicClient(request) {
    // Same gauntlet as the token endpoint — see the module header.
    const endpoint = validateDiscoveredEndpoint('registration_endpoint', request.registrationEndpoint, request.baseIsHttps, request.allowInternal ?? false);
    ...
    try {
        ...
        throw new Error(`decoding the registration response: ${err.message}`);
    }
    ...
    if (Array.isArray(echoed)) {
        ...
        if (extras.length > 0) {
            throw new Error(`registration response added redirect_uris we never requested (${JSON.stringify(extras)}) — a code-steal vector; refusing the registration`);
        }
    }
    return {
        clientId,
