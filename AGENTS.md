...
__This stack is built on the ngdpbase architecture throughout__ — configuration, request context, managers, providers — as agreed in [`docs/planning/architecture-principles-typescript.md`](https://github.com/jwilleke/yourphr/blob/main/docs/planning/architecture-principles-typescript.md) and tracked by [yourphr#608](https://github.com/jwilleke/yourphr/issues/608). Read the doc before changing `src/`. The invariant: a resource has exactly one manager and no other path reaches it; a request-scoped context says who is asking and is passed into managers; managers decide, providers return; configuration binds capabilities to providers (not configured = not loaded; required capabilities refuse to boot). A new capability is a manager with a config-bound provider; a new route is a handler taking `ctx`. Never a closure in `ServerModules`, never a raw query outside the owning manager, never an inline role check — that shape is what #608 removes.
