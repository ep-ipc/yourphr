...
- __ngdpbase uses no SQLite today__ — no `sqlite`, `sqlcipher`, `better-sqlite3`, `knex` or `pg` dependency, and no `node:sqlite` import anywhere in `src/`, `addons/` or `scripts/`. Counting dependencies was the wrong test, though: Node 24 ships SQLite built in (`node:sqlite`, `DatabaseSync`), so adding SQLite would cost ngdpbase __no__ dependency at all. What the built-in lacks is encryption — on Node v24.19.0 it is plain SQLite 3.53.3 and `PRAGMA cipher_version` returns nothing — so a store holding PHI still needs `better-sqlite3-multiple-ciphers`. __Encryption is the gap, not SQLite.__ Non-PHI stores (audit, sessions) could sit on the built-in with no new dependency. Every shipped *store of record* is file-based: `FileSystemProvider`, `VersioningFileProvider`, `FileUserProvider`, `FileAuditProvider`, `FileBackupProvider`; the cache, search and cloud-audit providers (`RedisCacheProvider`, `ElasticsearchSearchProvider`, `CloudAuditProvider`) are the exceptions.
...
| ngdpbase has no SQLite: 85 dependencies, none of them SQLite (2026-08-30) | Node 24 ships `node:sqlite`, so SQLite needs no dependency; the built-in has no cipher. Encryption is the gap, not SQLite (2026-09-21) |
