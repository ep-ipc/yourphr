...
| __tool__ — `read_health_metric` | the model, once it knows which metric to ask for | one wearable metric over a window, with the min/max/avg the server computed |
| __resources__ — summary, IPS summary, medications, conditions, allergies, immunizations, recent records, wearable health metrics | the PATIENT, from the attachment picker | the document itself, as JSON, exactly as the app's own screens read it |
...
Every resource is a GET that already has an access category, so the same default-deny gate decides it and the same access-log line records it. The list offered is the full eight and is __not__ filtered to the token's own scopes: an agent token cannot read its own record, and probing each route to find out would write an access-log line for a resource the patient never asked for. Reading one the token does not carry is refused by name — *"not given access to medications … mint one with the 'Medications' scope selected"* — and refused as a protocol __error__ rather than as content, so a reason can never be attached to a conversation as though it were the record.

__The wearable half is a second tool rather than a wider search__, because it is a second store. `search_records` runs over the FHIR resources a provider sent; HealthKit samples are written to `health_samples` directly and deliberately never through the FHIR write path, since every Observation write evaluates ~40 FHIRPath expressions in a fresh goja runtime and a year of five-minute heart rate would cost more than a day of CPU. A patient asking *"how has my sleep been?"* against a search-only bridge is told their record contains no sleep data — true of the resources, false of the instance, and exactly the answer a health record must not give. The catalog is published as a __resource__ (one row per metric, attachable, the same shape the app's own health page reads); the series is a __tool__, because a month of five-minute samples is thousands of points and only worth fetching once the model knows which metric it wants.

Then simply ask — *"search my records for metformin"*, *"when was my last tetanus shot?"*, *"how has my sleep been this week?"* — and check the access log afterwards, where the read appears under the token's name.
